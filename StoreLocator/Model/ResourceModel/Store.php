<?php
namespace Magenest\StoreLocator\Model\ResourceModel;

use Magento\Framework\Model\AbstractModel;
use Magenest\StoreLocator\Model\Store as ModelStoreLocator;
...
class Store extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('magenest_store_locator', 'id'); //store_locator <=> table_name
    }
    ...
    protected function _afterSave(AbstractModel $storeLocator)
    {
        ...
        $connection->delete($this->getTable('magenest_store_locator_store'), ['storelocator_id = ?' => $storeLocator->getId()]);
        ...
        foreach ($stores as $storeId) {
            ...
            $connection->insert($this->getTable('magenest_store_locator_store'), $data);
        }
        return $this;
    }
    ...
    public function getStores(ModelStoreLocator $storeLocator)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('magenest_store_locator_store'),
            'store_id'
        )->where(
